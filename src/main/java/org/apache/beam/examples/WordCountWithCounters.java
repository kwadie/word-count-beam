 ...
 *
 * <p>This class, {@link WordCountWithCounters}, is the second in a series of four successively more detailed
...
public class WordCountWithCounters {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(WordCountWithCounters.class);
  ...
  /**
   * Options supported by {@link WordCountWithCounters}.
   *
  ...
  public interface WordCountOptions extends PipelineOptions {
    ...
    /** Bucket to be created for the metrics output. */
    @Description("Bucket name to be created for the metrics, without gs:// prefix ")
    @Required
    String getBucket();
  ...
  static void runWordCount(WordCountOptions options) {
    ...
    storeMetrics(options, result);
  }

  static void storeMetrics(WordCountOptions options, PipelineResult result) {

    LOGGER.info("Start code after waitUntilFinish");
    String metricsLogs = "Metrics:\r\n";
    ...
    for (MetricResult<Long> counter : metricQueryResults.getCounters()) {
      String msg = counter.getName() + " : " + counter.getCommitted() + " / " + counter.getAttempted();
      LOGGER.info("PRINT METRIC " + msg);
      metricsLogs += msg;
      metricsLogs += msg + "\r\n";
    }
    for (MetricResult<DistributionResult> distribution : metricQueryResults.getDistributions()) {
      String msg = distribution.getName() + " : " + distribution.getCommitted().getMean();
      LOGGER.info("PRINT METRIC" + msg);
      metricsLogs += msg + "\r\n";
    }
    ...
    Bucket bucket = storage.create(BucketInfo.of(options.getBucket()));

    byte[] metricsLogsBytes = metricsLogs.getBytes(UTF_8);
    Blob blob = bucket.create("metrics.txt", metricsLogsBytes);

    LOGGER.info("Finish code after waitUntilFinish");
  }
